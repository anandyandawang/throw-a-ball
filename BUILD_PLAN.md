...
1. **Orientation**: own complementary filter — integrate gyro into quaternion `q`, nudge pitch/roll toward the accelerometer's gravity direction with a small gain. **No compass ever** (iOS doesn't fuse it, it drifts near electronics). Yaw is gyro-only in an arbitrary "game frame", anchored by the sync snapshot (§4.4): the canonical hold makes the device→hand transform a fixed known constant, and the sync button re-anchors yaw; the at-rest ZUPT then only corrects slow drift against the last sync.
...
### 4.4 Reference snapshot: canonical hold + sync button
...
- **M3 — live hand**: reference-snapshot sync (§4.4), pose streaming, desktop arm rig follows the phone in real time. *Accept: after syncing the canonical hold, swinging the phone swings the in-game hand with no visible lag or axis flips.*
